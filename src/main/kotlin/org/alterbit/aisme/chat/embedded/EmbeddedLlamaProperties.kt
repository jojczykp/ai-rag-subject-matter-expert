...
data class EnabledEmbeddedLlamaProperties(
    ...
    init {
        ...
        require(host.isNotBlank()) { "aisme.embedded-llama.config.host must not be blank" }
        require(port in 1..65535) { "aisme.embedded-llama.config.port must be between 1 and 65535" }
...
data class EmbeddedLlamaModelProperties(
    ...
    init {
        ...
        require(license.isNotBlank()) { "aisme.embedded-llama.config.models.license must not be blank" }
        require(hardwareRequirements.isNotBlank()) {
            "aisme.embedded-llama.config.models.hardware-requirements must not be blank"
        }
    }
