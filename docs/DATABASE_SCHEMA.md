...
`chunk_embedding.embedding` uses pgvector's variable-dimension `vector` type so
different enabled embedding models can store different vector sizes. The
`embedding_dimensions` column must match `vector_dims(embedding)`.
